...
public class ManifestDfzCollection {
	...
	public static void main(String[] args) throws IOException, ParseException {
		SpringApplication.run(ManifestDfzCollection.class, args);
		String quote = "\u005c\u0022";
		String books = "/data1/solr/ajax-dfz/books1.json";
		String pages = "/data1/solr/ajax-dfz/books_pages1.json";
		...
		for (int i = 0; i < booksObject.length(); i++) {
			...
			for (int j = 0; j < pagesObject.length(); j++) {
				...
				String position = (String) pagesObj.get("position");

				if (book_id.equals(books_id_pages)) {

					JSONObject json = readJsonFromUrl("http://b-dev998.pk.de:3003/xasia/dfz+"+books_id_pages+"+"+position+"/info.json");
					if (json!=null) {
						String manifest_id = "http://b-dev998.pk.de:3003/xasia/dfz+"+books_id_pages+"+"+position;
